...
function validCNP( $p_cnp ) {
    ...
    if( strlen( $p_cnp ) != 13 ) { return false; }
    $cnp = str_split( $p_cnp ); unset( $p_cnp );
    $hashTable = array( 2 , 7 , 9 , 1 , 4 , 6 , 3 , 5 , 8 , 2 , 7 , 9 );
    $hashResult = 0;
    // All characters must be numeric
    for( $i=0 ; $i<13 ; $i++ ) {
        if( !is_numeric( $cnp[$i] ) ) { return false; }
        $cnp[$i] = (int)$cnp[$i];
        if( $i < 12 ) { $hashResult += (int)$cnp[$i] * (int)$hashTable[$i]; }
    }
    unset( $hashTable , $i );
    ...
    switch( $cnp[0] ) {
        ...
        case 7  : case 8 : case 9 : { $year += 2000; if( $year > (int)date('Y')-14 ) { $year -= 100; } } break; // rezidenti si Cetateni Straini
        default : { return false; } break;
    }
    if( $year < 1800 || $year > 2099 ) { return false; } unset( $year );
